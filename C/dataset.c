...
void initDataSetExtra(DataSet* ds, Environment* envI) {
    ...
    for (index_type i = 0; i < MAX_LAYERS; i++) {
        for (index_type j = 0; j < MAX_POINTS_FOR_DATASET; j++) { //MAX_POINTS_FOR_DATASET is the max length of vector<Point> vect;
...
void addBoundaryPoint(DataSet* ds, float offset) {
    ...
    for (index_type i = 0; i < ds->env->num_layers; i++) { //num_layers is trapezoid_edges.size(), see environment.c
